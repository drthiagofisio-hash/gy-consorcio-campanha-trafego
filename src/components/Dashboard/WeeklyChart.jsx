...
export function CPLChart({ campanhaData }) {
  const fluxoColors = { bittrex: '#8b5cf6', grupoGT: '#14b8a6', davi: '#6366f1', desconhecido: '#94a3b8' };
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
      <h3 className="text-sm font-semibold text-gray-700 mb-4">CPL por Campanha (top 10 — menor é melhor)</h3>
      <div className="flex gap-4 mb-3 text-xs">
        <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm inline-block bg-purple-500"></span>WhatsApp</span>
        <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm inline-block bg-teal-500"></span>Form GT</span>
        <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm inline-block bg-indigo-500"></span>Form Davi</span>
      </div>
      <ResponsiveContainer width="100%" height={Math.max(200, data.length * 28)}>
        <BarChart data={data} layout="vertical" margin={{ top: 0, right: 60, left: 90, bottom: 0 }}>
          ...
          <Bar dataKey="cpl" name="CPL" radius={[0, 4, 4, 0]} fill="#3b82f6" />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
