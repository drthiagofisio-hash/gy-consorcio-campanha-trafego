          ...
          <table className="min-w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                {semanas.map(w => (
                  <th key={w} className={`px-4 py-3 text-center font-semibold ${wTextColors[w - 1]}`}>
                    Semana {w}
                  </th>
                ))}
                <th className="px-4 py-3 text-center font-semibold text-gray-600">Variação S1→S4</th>
              </tr>
            </thead>
              ...
              {metricas.map(m => (
                <tr key={m.key} className="hover:bg-gray-50">
                      ...
                      <td key={w} className={`px-4 py-3 text-center ${wColors[i]}`}>
                        ...
                        {w > 1 && (
                          <div className="flex justify-center mt-1">
                            <VarCell valor={var_} inverter={m.inverter} />
                          </div>
                        )}
                      </td>
                    );
                  ...
                  <td className="px-4 py-3 text-center">
                    <VarCell
                      valor={variacao(resumoPorSemana[4]?.[m.key], resumoPorSemana[1]?.[m.key])}
                      inverter={m.inverter}
                    />
                  </td>
                </tr>
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {CAMPANHAS.map(camp => {
                ...
                const temDados = cplPorSemana.some(v => v !== null);
                if (!temDados) return null;

                const var_ = variacao(cplPorSemana[3], cplPorSemana[0]);

                return (
                  <tr key={camp.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <span className="text-xs font-medium text-gray-800">{camp.nome}</span>
                    </td>
                        ...
                        <td key={i} className={`px-4 py-3 text-center ${wColors[i]}`}>
                          ...
                          {i > 0 && cpl && (
                            <div className="flex justify-center mt-0.5">
                              <VarCell valor={varSemana} inverter={true} />
                            </div>
                          )}
                        </td>
                      );
                    ...
                    <td className="px-4 py-3 text-center">
                      <VarCell valor={var_} inverter={true} />
                    </td>
                  </tr>
                );
              })}
